...
TCMPenalty = lambda * (1 - weighted TCM Balance) * coverage

FinalScore = BaseScore - TCMPenalty
```

TCM Balance는 온도, 맛, 귀경 세 축을 삼각형 UI로 조절합니다.

```text
weighted TCM Balance
= w_temp * temp_balance
+ w_taste * taste_diversity
+ w_meridian * meridian_coverage
```

결측된 TCM 속성은 coverage로 반영해 정보가 없는 조합이 과도하게 감점되지 않도록 했습니다.

## Data Sources

- RecipeDB: 실제 레시피 동시 등장 기반 Top20
- FlavorDB: 향미/분자 기반 Top20
- TCM Balance: 온도, 맛, 귀경 속성 기반 균형 점수
